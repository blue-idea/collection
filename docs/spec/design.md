...
本设计用于实现 `docs/spec/requirements.md` 2.2.0 定义的 Linkit MVP。总体目标如下：
...
4. 主题成员数为 0 时，空态区显示英文 CTA `Add bookmarks`，打开与工具栏相同的挑选器；可保留次要「新建书签」入口，但不自动将新建书签归属当前主题。
5. 单条 `Remove from collection` 立即生效；多选移出必须经确认对话框，确认前零副作用。
6. 移出只解除成员关系，书签记录保留在资料库；`Delete` 语义不变。
7. UI 文案使用英文：`Add bookmarks`、`Remove from collection` 等。
...
1. 解析平台默认 AppData 下的 `Linkit/` 引导根（bootstrap root）。
...
4. `data-root.json` 永远只保留在默认 AppData 引导根，不随资料库迁移到目标目录。

...
2. 菜单固定两项：`Show`（`runtime.Show` / `WindowShow` + 聚焦）、`Quit`（真正退出）。
...
- PR 与 main push 根据集中式影响映射执行受影响 E2E/视觉；文档和无 UI 影响的纯 Go 内部实现不安装 Playwright 浏览器。
- 未映射的前端生产代码与共享基础设施变更自动升级全量；定时或手动 CI 保留全量浏览器回归兜底。
- Vitest Coverage 单次执行同时承担单元/组件回归与覆盖率报告，禁止在同一 CI 中先后重复运行普通测试和 Coverage。
- Release 仅允许相同 SHA 已有成功 main CI 的提交进入；发布关键旅程通过后，多平台构建矩阵才可启动。
...
| Settings + i18n + Theme Tokens | REQ-019、REQ-023、REQ-028、REQ-029 |

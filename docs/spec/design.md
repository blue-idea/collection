...
- fix_task 1.15 中“新建标签”按现有模型解释为 New Bookmark：Tag 只有 `id`、`label`、`color`，而 Bookmark 已有 `thumbnail` 渐变键；因此不新增 Tag 字段或数据库迁移。
- `ui/src/config/thumbnail-gradients.ts` 集中维护示例数据已经使用的六种渐变映射与稳定键列表，现有预览组件继续消费同一配置，避免保存键与渲染键漂移。
- `ui/src/features/bookmarks/thumbnail.ts` 提供可注入随机源的纯函数；生产默认使用 `Math.random`，测试以边界随机值确定性验证首项、末项与越界保护。
- `NewBookmarkDialog` 仅在用户点击 Save 时生成一次 thumbnail，并随现有 `onCreate` payload 持久化；输入、Manual、Smart 与预览阶段均不提前写入。
- E2E 固定浏览器随机源验证保存后的详情渐变，并输出 Playwright Baseline/Actual/Diff 与 MCP 截图；不请求 AI 图片或第三方缩略图服务。
...
3. Windows 与 macOS 额外监听托盘图标双击：双击仅执行 `ShowMainWindow`，不自动打开 Settings；Linux 保持 best-effort，不将缺失双击能力伪报为 PASS。
4. Linux：best-effort；初始化失败时记录限制并继续运行，验收不得伪造 PASS。
5. 前端通过共享 Wails 事件适配器订阅 `linkit:open-settings`，React 卸载时调用订阅返回的取消函数；浏览器环境缺少 Wails runtime 时安全降级为 no-op。

#### 全局热键与应用内快捷键

1. **窗口显隐**必须走 Go `internal/hotkey` 系统级注册（窗口隐藏时 WebView `keydown` 不可用）；默认 `CmdOrCtrl+L`。
2. 其余快捷键（Spotlight / New / Insights / Settings / 视图 1–3 / 左侧 Sidebar / 右侧 Detail Panel）由前端 `useGlobalShortcuts` 按 `AppSettings.shortcuts` 生效；Esc 关闭浮层保持固定，不进入可配置集合。
...
2. 可配置 action：`spotlight`、`newBookmark`、`insights`、`settings`、`viewCard`、`viewList`、`viewMasonry`、`toggleLeftSidebar`、`toggleRightSidebar`、`toggleWindow`。
...
flowchart TD
  ...
  TraySettings[Tray Settings] --> Show[runtime.Show]
  TrayDClick[Tray Double Click] --> Show

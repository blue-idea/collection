...
- [ ] **TASK-008 · 实现原生导入导出文件服务**

  > 依赖：TASK-003、TASK-006 · 预计：2–3 小时

  - [ ] 实现原生打开/保存对话框、文件大小和 UTF-8 JSON 语法检查。
  - [ ] 实现导出信封、取消状态和不泄露设置/密钥的验证。
  - [ ] 覆盖有效、损坏、过大和用户取消路径。
...
- [ ] **TASK-009 · 实现安全网页元数据与外部 URL 服务**

  > 依赖：TASK-001、TASK-002 · 预计：3–4 小时

  - [ ] 实现 HTTP(S) 校验、超时、重定向、大小限制和静态 HTML 解析。
  - [ ] 实现系统浏览器打开 URL，只有成功后才允许更新访问计数。
  - [ ] 使用本地 HTTP 测试服务器覆盖失败和恶意输入。
...
- [ ] **TASK-010 · 完成本地模式启动、恢复与退出流程**

  > 依赖：TASK-005~007 · 预计：3–4 小时

  - [ ] 接入设置和本地 Repository，完成 loading gate 与本地模式入口。
  - [ ] 实现重启恢复、退出后保留本地数据和种子恢复二次确认。
  - [ ] 为损坏文件、备份可用和无数据状态编写组件/E2E 测试。
...
- [ ] **TASK-014 · 实现分类树 CRUD 与删除策略**

  > 依赖：TASK-005、TASK-011 · 预计：3–4 小时

  - [ ] 实现树构建、后代计数、创建、重命名和无环校验。
  - [ ] 实现移动内容后删除、递归删除和取消三种策略。
  - [ ] 接入分类树 UI 与破坏性确认对话框。
...
- [ ] **TASK-015 · 实现分类与书签拖拽**
  ...
  > 依赖：TASK-014 · 预计：3–4 小时

  - [ ] 接入 dnd-kit 分类排序和书签拖入分类。
  - [ ] 拒绝分类拖入自身或后代，并持久化合法 parentId。
  - [ ] 增加键盘拖拽或等价可访问操作。
...
- [ ] **TASK-018 · 实现标签维护、筛选与建议采纳**

  > 依赖：TASK-011、TASK-013 · 预计：2–3 小时

  - [ ] 实现标签唯一性、添加、移除和删除引用清理。
  - [ ] 接入侧栏计数、筛选和详情标签控件。
  - [ ] 实现采纳建议标签且不重复。
...
- [ ] **TASK-021 · 实现 Spotlight 关键词与 URL 流程**

  > 依赖：TASK-011、TASK-013 · 预计：3–4 小时

  - [ ] 实现搜索投影、关键词匹配、排序和空状态。
  - [ ] 接入 Cmd/Ctrl+K、结果定位、URL 检测和新增流程。
  - [ ] 覆盖标题、描述、域名、备注与大小写场景。
...
- [ ] **TASK-025 · 完成本地 MVP 关键旅程与视觉回归**

  > 依赖：TASK-010~024 · 预计：3–4 小时

  - [ ] 串联本地模式、入库、查找、整理、六视图、导入导出和设置旅程。
  - [ ] 验证三条核心路径均不超过三次主要操作。
  - [ ] 生成关键页面 Baseline、实际截图、Diff 和本地 E2E 报告。
...
- [ ] **TASK-028 · 实现 CloudRepository 与 revision 保存**

  > 依赖：TASK-005、TASK-006、TASK-026、TASK-027 · 预计：3–4 小时

  - [ ] 实现 load/create/save、Zod 校验和 user_id/session 二次检查。
  - [ ] 使用 expectedRevision 条件更新并正确映射零行冲突。
  - [ ] 云失败保留内存状态和 dirty cloud draft，不显示伪成功。
...
- [ ] **TASK-036 · 实现库内推荐与静态知识网络**

  > 依赖：TASK-016、TASK-018、TASK-032 · 预计：3–4 小时

  - [ ] 实现仅返回当前资料库 ID 的推荐和主题缺口建议。
  - [ ] 实现同标签、同主题、AI 相关的确定性径向 SVG 图。
  - [ ] 实现节点跳转与 AI 失败时的规则关联降级。
...
- [ ] **TASK-038 · 实现收藏洞察报告**
  ...
  > 依赖：TASK-013、TASK-016、TASK-018 · 预计：2–3 小时

  - [ ] 实现可追溯的统计与规则洞察卡片。
  - [ ] 实现行动项跳转到相关筛选、主题或健康视图。
  - [ ] 对空库、少量数据和 10,000 条数据覆盖测试。
...
- [ ] **TASK-041 · 验证 10,000 条性能预算**

  > 依赖：TASK-004、TASK-019~023、TASK-025、TASK-039 · 预计：3–4 小时

  - [ ] 在记录的参考环境运行热启动、搜索、筛选、视图切换和本地保存采样。
  - [ ] 验证网络操作 300ms 内出现 pending 状态。
  - [ ] 输出 P50/P95、样本数、构建模式和原始数据。

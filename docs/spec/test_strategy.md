...
PR 不默认执行全部 E2E，而是由 `config/test/e2e-impact.mjs` 将变更路径映射到功能域和测试文件：

- 所有 PR 固定执行 `tests/e2e/smoke.spec.ts`。
...
| Pull Request | Lint、类型、构建、单元测试、Smoke、受影响 E2E/视觉 | 阻止受影响范围回归 |
| main push | 全量 E2E 与视觉回归 | 建立可发布提交质量记录 |
| Release | 校验相同 SHA 的 main CI 成功，执行发布关键旅程，再构建多平台产物 | 阻止未经完整验证的提交发布 |
| 定时专项 | 性能、安全、Supabase 与长期全量回归 | 发现非功能与环境漂移 |

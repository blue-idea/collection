...
PR 与 main push 不默认执行全部 E2E，而是由 `config/test/e2e-impact.mjs` 将变更路径映射到功能域和测试文件：

- 文档、规格和无 UI 影响的纯 Go 内部实现不启动 Playwright，也不安装浏览器。
- 命中 UI 功能域时追加 `tests/e2e/smoke.spec.ts` 与该域的 E2E。
- 已映射的功能代码执行对应域的 E2E；UI、主题或全局样式变更追加对应视觉回归。
- Store、Repository、Schema、迁移、Playwright/构建配置、依赖锁文件和 CI 工作流等共享基础设施变更自动升级为全量 E2E 与视觉回归。
- 无法映射的生产代码变更按安全失败策略自动升级全量，不允许静默跳过。
- 直接修改 E2E 或视觉测试文件时，所修改的测试文件自动加入执行范围。
- 未执行且确认不受影响的用例标记为 `NOT_RUN_UNAFFECTED`，不得报告为 `PASS`。

| 阶段 | 测试范围 | 门禁作用 |
|------|----------|----------|
| 本地 TASK | 当前 TASK 的 Red/Green 测试与相关回归 | 阻止未验证实现 |
| Pull Request | Lint、类型、构建、单次 Coverage、受影响 E2E/视觉 | 阻止受影响范围回归 |
| main push | 单次 Coverage、Go 测试与受影响 E2E/视觉 | 建立可发布提交质量记录 |
| Release | 校验相同 SHA 的 main CI 成功，执行发布关键旅程，再构建多平台产物 | 阻止未经完整验证的提交发布 |
| 定时/手动全量 | 全量 E2E 与视觉回归 | 发现跨域回归和长期环境漂移 |

发布关键旅程固定包含 Smoke、本地核心路径和本地完整旅程；各平台构建任务不重复浏览器全量套件，只负责正式身份、产物、安装包和平台兼容门禁。
...
| 1.9.0 | 2026-07-21 | 已定稿 | 新增 J-17 与托盘/全局热键门禁；对齐 REQ-030 与 fix_task 1.8 |

...
| `theme` | enum | `midnight`、`ocean`、`graphite`、`sunset` |
...
| 文件 | 平台应用数据目录下路径 | 用途 |
|------|------------------------|------|
| 正式资料库 | `library.json` | 当前 LibraryEnvelope |
| 备份资料库 | `library.json.bak` | 上一次有效正式版本 |
| 临时资料库 | `library.json.tmp` | 原子写入中间文件 |
| 云未同步草稿 | `cloud-draft.json` | 云模式下最新未确认同步状态 |
| 设置 | `settings.json` | AppSettings |

具体 AppData 根目录由 Wails/Go 平台 API 获取，不硬编码用户目录。

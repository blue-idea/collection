...
func buildAnalyzeBookmarkSystemPrompt(locale string) string {
	languageRule := "Write title, description, summary, and suggestedTags entirely in English."
	if strings.EqualFold(strings.TrimSpace(locale), "zh") {
		languageRule = "Write title, description, summary, and suggestedTags entirely in Simplified Chinese (简体中文)."
	}
	...
	return "You are Linkit's bookmark analyzer. Reply with a single JSON object only. " +
		...
		"suggestedTags are short label strings. Do not wrap in markdown. " +
		languageRule +

...
export function ExploreDialog({ recommendations, themeGaps, bookmarkLabels, collectionLabels, onClose, onSelect, onConfirmGap }: {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4">
    <div role="dialog" aria-modal="true" aria-label={i18n.t('explore.title')} className="glass-strong w-full max-w-xl rounded-mac-xl p-5 ring-glow space-y-4">
      <div className="flex justify-between gap-3"><div><h2 className="text-[16px] font-semibold text-ink-100">{i18n.t('explore.title')}</h2>
        <p className="text-[11px] text-ink-400">{i18n.t('explore.hint')}</p></div>
        <button aria-label={i18n.t('explore.close')} onClick={onClose} className="text-ink-400">×</button></div>
      <section><h3 className="text-[12px] font-semibold text-ink-200 mb-2">{i18n.t('explore.related')}</h3>
      ...
      <section><h3 className="text-[12px] font-semibold text-ink-200 mb-2">{i18n.t('explore.gaps')}</h3>
        {themeGaps.length === 0 ? <p className="text-[11px] text-ink-400">{i18n.t('explore.noGaps')}</p> : <div className="space-y-2">{themeGaps.map((gap) => {
          ...
          return <div key={`${gap.collectionId}:${gap.bookmarkId}`} className="flex items-center justify-between gap-2 rounded-lg bg-ink-800/50 p-3 hairline">
            ...
            <button aria-label={i18n.t('explore.addTo', { bookmark, collection })} onClick={() => onConfirmGap(gap.collectionId, gap.bookmarkId)}
              className="rounded-md bg-accent-500 px-2.5 py-1.5 text-[11px] font-semibold text-white">{i18n.t('common.add')}</button>
          </div>;
        })}</div>}
      </section>
    </div>

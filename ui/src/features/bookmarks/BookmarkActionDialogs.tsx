...
export function BookmarkEditorDialog({ bookmark, categories, tags, collections, onClose, onSave }: {
  ...
  return <DialogShell label="Edit bookmark" onClose={onClose}>
    ...
    <div className="mt-4 grid grid-cols-2 gap-3">
      ...
      <label className="text-xs text-ink-300">Category<select aria-label="Category" value={values.categoryId} onChange={(e) => setValues({ ...values, categoryId: e.target.value })} className={inputClass}><option value="">Uncategorized</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
      <label className="text-xs text-ink-300">Read status<select aria-label="Read status" value={values.readStatus} onChange={(e) => setValues({ ...values, readStatus: e.target.value as typeof values.readStatus })} className={inputClass}>{['unread', 'reading', 'read', 'archived'].map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
...
export function BookmarkMoveDialog({ open, count, categories, onClose, onMove }: { open: boolean; count: number; categories: Category[]; onClose: () => void; onMove: (categoryId: string | null) => void }) {
  ...
  if (!open) return null;
  return <DialogShell label="Move bookmarks" onClose={onClose}><h2 className="text-lg font-semibold text-ink-100">Move {count} bookmark{count === 1 ? '' : 's'}</h2><label className="mt-4 block text-xs text-ink-300">Target category<select aria-label="Target category" value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className={inputClass}><option value="">Uncategorized</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><div className="mt-5 flex justify-end gap-2"><Button onClick={onClose}>Cancel</Button><Button variant="primary" onClick={() => onMove(categoryId || null)}>Move bookmarks</Button></div></DialogShell>;
}

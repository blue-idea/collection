...
export function BookmarkEditorDialog({ bookmark, categories, tags, collections, onClose, onSave }: {
  ...
  return <DialogShell label={i18n.t('bookmark.edit')} onClose={onClose}>
    ...
    <div className="mt-5 flex justify-end gap-2"><Button onClick={onClose}>{i18n.t('common.cancel')}</Button><Button variant="primary" onClick={() => onSave({ ...values, categoryId: values.categoryId || null })}>{i18n.t('bookmark.editor.save')}</Button></div>
  </DialogShell>;
}
